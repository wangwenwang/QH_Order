...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (view == nil) {
        // 转换坐标系
        CGPoint newPoint = [_deleteButton convertPoint:point fromView:self];
        // 判断触摸点是否在button上
        if (CGRectContainsPoint(_deleteButton.bounds, newPoint)) {
            view = _deleteButton;
        }
    }
    return view;
...
- (void)layoutSubviews {
    ...
    _deleteButton.frame = CGRectMake(self.bounds.size.width - ACMediaDeleteButtonWidth / 2, -(ACMediaDeleteButtonWidth / 2), ACMediaDeleteButtonWidth, ACMediaDeleteButtonWidth);
