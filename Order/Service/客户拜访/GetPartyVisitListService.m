...
#define kAPI_NAME_GetPartyVisitHistory @"获取历史客户拜访列表"
...
- (void)GetPartyVisitHistory:(nullable NSString *)strBusinessIdx andStrPartyCode:(nullable NSString *)strPartyCode andStartTime:(nullable NSString *)startTime andEndTime:(nullable NSString *)endTime {
    ...
    NSLog(@"接口:%@请求%@参数：%@", kAPI_NAME_GetPartyVisitHistory, API_GetPartyVisitHistory, parameters);
    
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"text/html"];
    [manager POST:API_GetPartyVisitHistory parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        nil;
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"%@|请求成功---%@", kAPI_NAME_GetPartyVisitHistory, responseObject);
        ...
        if([responseObject isKindOfClass:[NSDictionary class]]) {
            ...
            if(type == 1) {
                ...
                if(getPartyVisitListM.getPartyVisitItemModel.count < 1) {
                    ...
                    [self successOfGetPartyVisitHistory_NoData];
                } else {
                    
                    [self successOfGetPartyVisitHistory:getPartyVisitListM];
                }
            } else if(type == -2) {
                ...
                [self failureOfGetPartyVisitHistory:msg];
            }
        } else {
        ...
        NSLog(@"%@|请求失败:%@", kAPI_NAME_GetPartyVisitHistory, error);
        [self failureOfGetPartyVisitHistory:[NSString stringWithFormat:@"请求%@失败", kAPI_NAME_GetPartyVisitList]];
    }];
}


// 成功
- (void)successOfGetPartyVisitHistory:(GetPartyVisitListModel *)checkStockListM {
    
    if([_delegate respondsToSelector:@selector(successOfGetPartyVisitHistory:)]) {
        
        [_delegate successOfGetPartyVisitHistory:checkStockListM];
    }
}
...
- (void)successOfGetPartyVisitHistory_NoData {
    ...
    if([_delegate respondsToSelector:@selector(successOfGetPartyVisitHistory_NoData)]) {
        
        [_delegate successOfGetPartyVisitHistory_NoData];
    }
}
...
- (void)failureOfGetPartyVisitHistory:(nullable NSString *)msg {
    ...
    if([_delegate respondsToSelector:@selector(failureOfGetPartyVisitHistory:)]) {
        
        [_delegate failureOfGetPartyVisitHistory:msg];
    }
}
